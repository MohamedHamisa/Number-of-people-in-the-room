...
while True:
    ...
    if new_pts.ravel()[2]  <= 20:
        new_pts.ravel()[2] = 20   #ravel is a module used to flatten the original array not a copy like flatten
    ...
    x,y = new_pts[0,:,:].ravel()
    a,b = new_pts[1,:,:].ravel()
    cv2.line(new_inp_img, (int(x),int(y)), (int(a),int(b)), (0,0,255), 15)  #line to make the point that will specify the person is going or coming
    ...
    if new_pts.ravel()[0]  > 400 or new_pts.ravel()[2]  > 400:        
        if new_pts.ravel()[0] > 550 or new_pts.ravel()[2]  > 550:
            ...
            cv2.putText(outp, "gone at {}".format(datetime.datetime.now().strftime("%H:%M")), (10,ytest_pos), 
                cv2.FONT_HERSHEY_PLAIN, 3, (0,255,0))  #the output will be in format of time
    ...
    old_pts = new_pts.reshape(-1,1,2)  #update the points
